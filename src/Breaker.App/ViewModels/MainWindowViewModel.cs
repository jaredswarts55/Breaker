...
namespace Breaker.App.ViewModels
{
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        public void SetFocus(Expression<Func<Object>> selector)
        {
        }
        ...
        private async Task RunSlashCommand(SlashCommand slashCommand, bool closeWindow = true)
        {
            ...
            if (closeWindow)
                ClearSearchAndHide();
            await _mediator.Send(slashCommand.CreateRequest(slashCommand.SubstituteCommandText ?? commandText,
                _settings));
        }
        ...
        public async void CopyItem(SearchEntry eventArgs)
        {
            if (eventArgs is SlashCommand slashCommand)
            {
                ...
                _clipboardService.SetText(toClipboard);
            }
            else
                _clipboardService.SetText(eventArgs.Path);
        }
        ...
        private async Task InternalSearch(CancellationToken cancellationToken)
        {
            ...
            if (SearchText.StartsWith("/"))
            {
                ...
                if (slashCommands.Any())
                {
                    ...
                    {
                        await Dispatcher.UIThread.InvokeAsync(ClearFoundItemsAndHeader);
                    }
